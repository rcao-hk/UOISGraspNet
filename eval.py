...
def eval_scene(scene_idx, cfgs):
    ...
    for frame_idx in range(256):
        ...
        pred_mask_path = os.path.join(dataset_root, '{}_mask/scene_{:04}/{}/{:04}.png'.format(segment_method, scene_idx, camera, frame_idx))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--segment_method', default='GDS_v0.1.1', help='Segmentation method [uois/uoais/GDS]')
    cfgs = parser.parse_args()

    scene_list = list(range(100, 130))
    # scene_list = list(range(130, 160))
    # scene_list = list(range(160, 190))
    result_list = parallel_eval(scene_list, cfgs=cfgs, proc=10)
